  ...
  //Also note that that method should be retuning a promise, so you could use .then in this function.
    ...
    $scope.getSongData = function() {
      itunesService.getSongs($scope.artist).then(function (songs){
        $scope.songData = songs.map(function (itunesFormattedResults){
          return {
            AlbumArt: itunesFormattedResults.artworkUrl100,
            Artist: itunesFormattedResults.artistName,
            Collection: itunesFormattedResults.collectionName,
            CollectionPrice: itunesFormattedResults.collectionPrice,
            Play: itunesFormattedResults.previewUrl,
